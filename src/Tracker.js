...
const _config = require(path.join(process.env.SRC_PATH, 'config.json'));
...
class Tracker {
    ...
    initPingPong() {
        ...
        if (global.vaclient.tracker.pingpong) {
            self.ping = global.vaclient.tracker.pingpong 
        } else {
            //do initial ping
            self.options.mainWindow.webContents.send('ping-pong', self.options.user.id);

            //3 minutes hard coded
            self.ping = Utils.childTimer({
            interval: config.PING_SECONDS
            }, function () {
                self.options.mainWindow.webContents.send('ping-pong', self.options.user.id);
            });

            self.ping.start();
            global.vaclient.tracker.pingpong = self.ping;
        }
    }
    ...
    clearTrackerTimers() {
        ...
        if (global.vaclient.tracker.pingpong) {
            global.vaclient.tracker.pingpong.stop();
            global.vaclient.tracker.pingpong = null;
        }
    }
     ...
     */
    screenshot() {
        console.log('attempt to screenshot');
        let self = this;
        ss.all().then(function (imgBuffers) {
            for(var i in imgBuffers) {
                    console.log('sending to IPC');
                    self.options.mainWindow.webContents.send('upload-screenshot', imgBuffers[i].toString('base64'), self.mouseActivity, self.keyboardActivity); 
            }
    ...
    startIdleTracking() {
        ...
        this.idleHook.onIdle(function () {
            self.options.renderView(`file://${__dirname}/resources/views/auth/idle.html.twig`, { resPath: process.env.RES_PATH, appVer: process.env.PACKAGE_VER, idleTimeoutSeconds: self.idle_timeout, accessToken: process.env.ACCESS_TOKEN, appEnv: process.env.PACKAGE_ENV });
