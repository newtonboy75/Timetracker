...
const _config = require(path.join(srcPath, 'config.json'));
const config = _config.env[_config.default];
const API_HOST = config.HOST;
const API_PORT = config.PORT;
const SOCKET_HOST = config.SOCKET_HOST;
...
class VA {
	...
	createWindow() {
		...
		this.renderView(`file://${__dirname}/resources/views/auth/connect.html.twig`, { resPath: process.env.RES_PATH, appVer: process.env.PACKAGE_VER, appEnv: process.env.PACKAGE_ENV, recon: 0 });
	}
	...
	ipcEvents() {
		...
		ipcMain.on('open-task-window', function (event, data) {
			...
			let taskWidth = 400, taskHeight = 40, x = (width - taskWidth) / 2, y = (height - taskHeight);
			if (typeof self.taskWindow !== 'undefined' && self.taskWindow !== null) {
				self.taskWindow.destroy();
			}
			...
			self.renderView(`file://${__dirname}/resources/views/pages/current-task.html.twig`, {
				...
				appEnv: process.env.PACKAGE_ENV,
				task: data.name,
		...
		ipcMain.on('render-view', function (event, view, twigData) {
			...
			/** if (view.includes('login.html.twig')) {
				self.mainWindow.setSize(600,600);
				self.mainWindow.center();
			}

			if (view.includes('dashboard.html.twig')) {
				self.mainWindow.setSize(1025,600);
				self.mainWindow.center();
			} **/
	...
	socketEvents() {
		...
		this.socket.on('disconnect', (reason) => {
			...
			self.renderView(`file://${__dirname}/resources/views/auth/connect.html.twig`, { resPath: process.env.RES_PATH, appVer: process.env.PACKAGE_VER, appEnv: process.env.PACKAGE_ENV, recon: 0 });
			// add checking if global.va.timers exists
			if (typeof global.va.timers !== 'undefined' && global.va.timers !== null && global.va.timers.forEach === "function") {
		...
		this.socket.on('connect_error', (err) => {
			// console.log('err', err);
			 ...
			 if (typeof self.taskWindow !== 'undefined' && self.taskWindow !== null) {
				 console.log('destroy task window');
				self.taskWindow.destroy();
			}
			...
			if (self.mainWindow!=null) {
				...
				self.mainWindow.webContents.focus();
			}
		});
	}
